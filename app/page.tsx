...
function makePowderHourly(day, climb, elevationM) {
  return makeCornHourly(day, climb, elevationM, "powder").map((h) => ({ ...h, powderState: powderStateForHour(h) }));
}
function analyzePowderDay(climb, day, avalanche) {
  const profile = routeElevationProfile(climb);
  const elevationM = profile.entranceM || climb.summitM;
  const hourly = makePowderHourly(day, climb, elevationM);
  ...
  const heavyHours = hourly.filter((h) => h.powderState === "Getting heavy").length;
  const maxMelt = Math.max(...hourly.map((h) => h.meltIntegralFhrs || 0));
  const maxFreeze = Math.max(...hourly.map((h) => h.freezeIntegralFhrs || 0));
  let score = 10;
  if (snow24 < (climb.thresholds.minNewSnow24hCm ?? 8)) score -= 2.0;
  ...
  if (preservedHours >= 10) score += 1.0;
  ...
  if (snow24 > 20) interpretation = "No-go signal: more than 20 cm new snow in the latest 24h block creates too much rapid load for this steep continental-snowpack objective.";
  ...
  else if (powderScore >= 7.5) interpretation = "Good powder-preservation signal: cold snow, manageable new load, freezing level below the route, and limited heat input. Best timing is after storm loading has eased, not during peak snowfall.";
  ...
  return { day, profile, hourly, snow24, wind, summitTemp, freezingLevel, preservedHours, heavyHours, maxMelt, maxFreeze, powderScore, interpretation };
}
function analyzePowderCycle(climb, forecast = [], avalanche) {
  if (climb.routeType !== "powder_ski" || !forecast.length) return null;
  const days = forecast.slice(0, 7).map((day) => analyzePowderDay(climb, day, avalanche));

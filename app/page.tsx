...
function makeCornHourly(day, climb, elevationM, kind) {
  ...
  const pressureBonus = (day.pressureHpa || 1013) >= 1018 ? 0.8 : 0;
  const precip = dayPrecip(day);
  const wind = dayWind(day);
  const solarBoost = kind === "entrance" ? 0.7 : 1.1;
  return Array.from({ length: 16 }, (_, idx) => {
    const hour = idx + 5;
    const dayCurve = Math.sin(Math.max(0, Math.min(Math.PI, ((hour - 6) / 12) * Math.PI)));
    const temp = low + (high - low) * dayCurve;
    const effective = temp + dayCurve * solarBoost + pressureBonus - Math.max(0, wind - 25) * 0.035 - precip * 0.5;
    const meltIntegral = Math.max(0, effective) * Math.max(0, hour - 6) * 0.35;
    let state = "Frozen";
    if (effective >= -0.5 && effective <= 3.5 && meltIntegral >= 1 && meltIntegral <= 12) state = "Prime corn";
    if (effective > 3.5 || meltIntegral > 12) state = "Too soft / wet";
    if (effective < -0.5 && hour >= 11) state = "Icy / delayed";
    return { hour, label: `${hour}:00`, effectiveTempC: Number(effective.toFixed(1)), airTempC: Number(temp.toFixed(1)), meltIntegral: Number(meltIntegral.toFixed(1)), state };
  });
}
...
function CornCyclePanel({ climb, forecast }) {
  ...
  return <div className="space-y-4"><div className="grid gap-4 md:grid-cols-4"><MetricCard icon={SunIcon} label="Corn Score" value={`${analysis.cornScore.toFixed(1)} / 10`} detail="Refreeze + thaw + wind/precip" className={scoreColor(analysis.cornScore)} /><MetricCard icon={SnowIcon} label="Refreeze Score" value={`${analysis.refreezeScore.toFixed(1)} / 10`} detail="Overnight supportability" className={scoreColor(analysis.refreezeScore)} /><MetricCard icon={MountainIcon} label="Ski Entrance" value={analysis.targetSki ? `${analysis.targetSki}:00` : "No clear time"} detail={`${analysis.profile.entranceM} m · ${analysis.profile.aspect}`} /><MetricCard icon={TrendIcon} label="Exit Target" value={analysis.exitTarget ? `${analysis.exitTarget}:00` : "No clear time"} detail={`${analysis.profile.exitM} m lower route`} /></div><Card><CardContent className="p-5"><h3 className="font-semibold">Corn-cycle interpretation</h3><p className="mt-2 text-slate-700">{analysis.interpretation}</p><p className="mt-2 text-sm text-slate-500">This is a heuristic Ullr-style surface-state model: effective snow temperature is estimated from air temperature, solar timing, pressure/cloud proxy, wind penalty, and precip penalty. Treat as timing guidance, not a safety decision.</p></CardContent></Card><div className="grid gap-4 lg:grid-cols-2"><CornCycleGraph title="Entrance / high-point corn graph" elevationM={analysis.profile.entranceM} rows={analysis.entrance} /><CornCycleGraph title="Exit / lower-route corn graph" elevationM={analysis.profile.exitM} rows={analysis.exit} /></div><Card><CardContent className="p-5"><h3 className="mb-2 font-semibold">Route timing strategy</h3><div className="grid gap-3 md:grid-cols-4 text-sm"><div className="rounded-xl bg-slate-50 p-3"><div className="text-slate-500">Start climbing</div><div className="text-xl font-semibold">{analysis.startClimb ? `${analysis.startClimb}:00` : "—"}</div></div><div className="rounded-xl bg-slate-50 p-3"><div className="text-slate-500">Reach entrance</div><div className="text-xl font-semibold">{analysis.targetSki ? `${Math.max(analysis.targetSki - 1, 6)}:00` : "—"}</div></div><div className="rounded-xl bg-emerald-50 p-3"><div className="text-emerald-700">Start skiing</div><div className="text-xl font-semibold text-emerald-900">{analysis.targetSki ? `${analysis.targetSki}:00` : "—"}</div></div><div className="rounded-xl bg-slate-50 p-3"><div className="text-slate-500">Clear lower route</div><div className="text-xl font-semibold">{analysis.exitTarget ? `${analysis.exitTarget}:00` : "—"}</div></div></div></CardContent></Card></div>;
}

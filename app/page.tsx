...
function cleanPct(value) { return typeof value === "number" && Number.isFinite(value) ? Math.max(0, Math.min(100, value)) : null; }
function dayCloudPct(day) { return cleanPct(day.cloudCoverPct ?? day.cloudPct ?? day.cloudCover); }
function dayLowCloudPct(day) { return cleanPct(day.cloudCoverLowPct ?? day.lowCloudPct ?? day.lowCloudCover); }
function dayMidCloudPct(day) { return cleanPct(day.cloudCoverMidPct ?? day.midCloudPct ?? day.midCloudCover); }
function dayHighCloudPct(day) { return cleanPct(day.cloudCoverHighPct ?? day.highCloudPct ?? day.highCloudCover); }
...
function cloudWeightedFraction(day, fallback = null) {
...
function navigationVisibilityScore(day) {
  ...
  const total = dayCloudPct(day) ?? Math.max(low, dayMidCloudPct(day) ?? 0, dayHighCloudPct(day) ?? 0);
...
function makeCornHourly(day, climb, elevationM, kind) {
  ...
  const cloud = cloudWeightedFraction(day, 0.35);

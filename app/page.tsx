...
function getTopDrivers(climb, forecast = [], history = [], avalanche = {}) {
  ...
  if (surface.hasData) {
    ...
    if (surface.freezeThawCycles >= 4 && climb.routeType !== "powder_ski") positives.push({ label: "Consolidation history", detail: `${surface.freezeThawCycles} freeze/thaw cycles in 14d`, severity: "positive" });
    if (surface.warmPulseSeverity >= 7) constraints.push({ label: "Warm-pulse history", detail: `Warm-pulse severity ${surface.warmPulseSeverity.toFixed(1)}`, severity: "moderate" });
  }

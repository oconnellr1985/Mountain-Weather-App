...
function dayCloudPct(day) {
  const fromModels = avg((day.modelValues || []).map((m) => m.cloudCoverPct).filter((v) => typeof v === "number" && Number.isFinite(v)));
  return pct(firstNumber(day.cloudCoverPct, day.cloudPct, day.cloudCover, day.totalCloudPct, day.totalCloudCoverPct, fromModels || null), 35);
}
function dayLowCloudPct(day) {
  const fromModels = avg((day.modelValues || []).map((m) => m.cloudCoverLowPct).filter((v) => typeof v === "number" && Number.isFinite(v)));
  return pct(firstNumber(day.cloudCoverLowPct, day.lowCloudPct, day.cloudCoverLow, day.lowCloudCoverPct, fromModels || null), dayCloudPct(day));
}
function dayMidCloudPct(day) {
  const fromModels = avg((day.modelValues || []).map((m) => m.cloudCoverMidPct).filter((v) => typeof v === "number" && Number.isFinite(v)));
  return pct(firstNumber(day.cloudCoverMidPct, day.midCloudPct, day.cloudCoverMid, day.midCloudCoverPct, fromModels || null), Math.max(0, dayCloudPct(day) - dayLowCloudPct(day)) * 0.5);
}
function dayHighCloudPct(day) {
  const fromModels = avg((day.modelValues || []).map((m) => m.cloudCoverHighPct).filter((v) => typeof v === "number" && Number.isFinite(v)));
  return pct(firstNumber(day.cloudCoverHighPct, day.highCloudPct, day.cloudCoverHigh, day.highCloudCoverPct, fromModels || null), Math.max(0, dayCloudPct(day) - dayLowCloudPct(day) - dayMidCloudPct(day)));
}
function cornCloudFraction(day) {
  // Low cloud kills direct solar most strongly; high thin cloud is often much less destructive.
  return Math.max(0, Math.min(1, (dayLowCloudPct(day) * 0.7 + dayMidCloudPct(day) * 0.25 + dayHighCloudPct(day) * 0.05) / 100));
}
function solarReliability(day) {
  const low = dayLowCloudPct(day);
  const mid = dayMidCloudPct(day);
  const high = dayHighCloudPct(day);
  return Math.max(0.15, Math.min(1, 1 - low * 0.008 - mid * 0.004 - high * 0.0015));
}
function navigationVisibilityScore(day) {
  ...
  return clampScore(10 - low * 0.07 - total * 0.025 - precip * 0.8 - Math.max(0, wind - 35) * 0.08);
}
...
function scoreWindowForDays(climb, forecastDays = []) {
  ...
  if (routeType === "powder_ski") {
    ...
    routePenalty += forecastDays.reduce((s, d) => {
      ...
      const visibility = navigationVisibilityScore(d);
      if (visibility < 5) p += 2.2;
      else if (visibility < 7) p += 0.9;
      return s + p;
    }, 0);
  } else {
...
function buildSurfaceState(climb, history = []) {
  ...
  if (!days.length) return { hasData: false, days: [], snowLoadingIndex: 0, snow3d: 0, snow7d: 0, snow14d: 0, rain14d: 0, freezeThawCycles: 0, warmPulseSeverity: 0, surfaceDamage: 0, surfaceRecovery: 0, routePhase: "NO DATA", interpretation: "No observed 14-day history supplied by the backend yet." };
  ...
  const freezeThawCycles = days.filter((d) => historyTempMinC(d) < -1 && historyTempMaxC(d) > 1).length;
  const warmPulseSeverity = days.reduce((s, d) => { const fl = historyFreezingLevelM(d); const summitExcess = Math.max(0, fl - climb.summitM); const tempExcess = Math.max(0, historyTempMaxC(d)); return s + summitExcess / 400 + tempExcess * 0.8 + historyRainMm(d) * 0.6; }, 0);
  const windStripping = days.reduce((s, d) => s + Math.max(0, historyWindKph(d) - 35) * 0.08, 0);
  const surfaceDamage = clampScore(warmPulseSeverity * 0.55 + rain14d * 0.3 + Math.max(0, snow3d - 20) * 0.08);
  const surfaceRecovery = clampScore(freezeThawCycles * 0.8 + days.filter((d) => historyTempMinC(d) < -5).length * 0.35 + windStripping * 0.2);
  ...
  if (climb.routeType === "mixed_alpine_ice") {
    ...
    else if (freezeThawCycles >= 5 && surfaceDamage <= 5 && snow7d <= 10) routePhase = "prime neve / consolidation signal";
    else if (warmPulseSeverity >= 7 || rain14d > 5) routePhase = "warm-pulse affected / deterioration concern";
    else if (surfaceRecovery >= 6 && surfaceDamage <= 6) routePhase = "improving alpine surface";
  ...
  if (climb.id === "robson-kain") interpretation += `For the Kain Face, the useful signal is consolidation without recent loading: ${freezeThawCycles} freeze/thaw cycles, ${snow7d.toFixed(1)} cm snow in 7d, warm-pulse severity ${warmPulseSeverity.toFixed(1)}, surface damage ${surfaceDamage.toFixed(1)}, recovery ${surfaceRecovery.toFixed(1)}.`;
  else interpretation += `14d snow ${snow14d.toFixed(1)} cm, 7d snow ${snow7d.toFixed(1)} cm, rain ${rain14d.toFixed(1)} mm, freeze/thaw cycles ${freezeThawCycles}, warm-pulse severity ${warmPulseSeverity.toFixed(1)}.`;
  return { hasData: true, days, snowLoadingIndex, snow3d, snow7d, snow14d, rain14d, freezeThawCycles, warmPulseSeverity, surfaceDamage, surfaceRecovery, routePhase, interpretation };
}
...
export default function MountainWindowApp() {
  ...
  const recommendation = useMemo(() => {
    ...
    if ((climb.routeType === "mixed_alpine_ice" || climb.routeType === "pure_rock") && bestWindow.quality >= 6.5 && todayVisibility < 6) return `${climb.name}: weather ingredients are partly favorable, but cloud cover creates ${cloudNavigationLabel(todayVisibility).toLowerCase()} for route finding/glacier navigation. Low cloud ${Math.round(dayLowCloudPct(forecast[0] || {}))}%, total cloud ${Math.round(dayCloudPct(forecast[0] || {}))}%.`;
    if (climb.routeType === "ski_corn" && bestWindow.quality >= 6.5 && todaySolar < 0.55) return `${climb.name}: temperature/freezing-level signal may be acceptable, but cloud cover reduces solar reliability to ${Math.round(todaySolar * 100)}%. Corn timing may be delayed, muted, or fail to soften.`;
